...
void pdImageUtils_GetBayerDither(
    uint8_t **buffer, uint8_t row, uint8_t rb, PDImageUtilsBayerType type, uint8_t threshold
) {
    ...
    if (!is_threshold_valid) {
        s_pd->system->error("Threshold of %d is not valid for bayer filter type %d.", threshold, type);
    }
    ...
    for (int r = 0; r < row; r++) {
        for (int c = 0; c < rb; c++) {
            for (int bc = 0; bc < 8; bc++) {
                switch (type) {
                    case PDImageUtilsBayer2:
                        buffer[r * rb + c] += ((threshold > bayer2x2in8[r % 8][bc] ? 1 : 0) << (8 - bc));
                        break;
                    case PDImageUtilsBayer4:
                        buffer[r * rb + c] += ((threshold > bayer4x4in8[r % 8][bc] ? 1 : 0) << (8 - bc));
                        break;
                    case PDImageUtilsBayer8:
                        buffer[r * rb + c] += ((threshold > bayer8x8[r % 8][bc] ? 1 : 0) << (8 - bc));
                        break;
                    default:
                        break;
                }
